...
import { getMission } from "../../../db/actions/missionActions.js"
...
const onConfirm = (interaction: ChatInputCommandInteraction, target: User, isUniversal: boolean, category: string, index: number, mission: Mission) =>
    async (buttonInteraction: ButtonInteraction) => {
        ...
        const result = firebaseDb.runTransaction(async transaction => {
            const getMissionResult = await getMission(interaction.user.id, isUniversal ? interaction.user.id : target.id, category, index, transaction)
            if (getMissionResult === undefined || getMissionResult === null)
            ...
            const { mission, missionDocRef } = getMissionResult

            transaction.update(missionDocRef, { completed: FieldValue.arrayUnion(target.id) })
            transaction.update(missionProgressDocRef, { currentScore: FieldValue.increment(mission.score) })
        })
