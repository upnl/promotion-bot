...
const onConfirm =
  (
    ...
    chiefRole: Role | APIRole,
    regularRole: Role | APIRole,
    associateRole: Role | APIRole
  ) =>
  async (buttonInteraction: ButtonInteraction) => {
    ...
    try {
      await setRoleIds({
        chiefRole: chiefRole.id,
        regularRole: regularRole.id,
        associateRole: associateRole.id,
      });
...
export const doReply = async (
  ...
  const reply = await interaction.editReply({
    embeds: [
      EmbedBuilder.from(initializeEmbedPrototype)
        .addFields({ name: "넬장", value: chiefRole.toString(), inline: true })
        .addFields({
          name: "정회원",
          value: regularRole.toString(),
